# Comprehensive historical analysis of Formula 1 team performance
#spanning from 2010 to 2024. Our analysis focuses on two key aspects: total points scored by
#teams over the years and the number of constructor championships won by each team.
...
process_constructor_standings <- function(data) {
  ...
  for (i in seq_along(constructor_standings)) {
    ...
    if (constructor_standings[[i]]$Constructor$name %in% c("Force India", "Aston Martin", "Racing
 Point")) {
  ...
  constructor_df <- data.frame(
    Season = rep(as.integer(data$MRData$StandingsTable$StandingsLists[[1]]$season),
                 length(constructor_standings)),
...
teams_to_include <- c("Alpine", "Aston Martin", "Red Bull", "Ferrari", "McLaren", "Williams",
                      "Mercedes", "Haas", "RB", "Sauber")
filtered_data <- constructor_standings_data %>%
  filter(Team %in% teams_to_include)
# Customize colors for each team
# Customize colors for each team
team_colors <- c("Alpine" = "#fa57a9", "Aston Martin" = "dark green", "Red Bull" = "navy",
...
for (season in 1950:2023) {
  ...
  if (winning_team %in% championship_counts$Team) {
    ...
    # Addthe winning team to the championship_counts data frame
    championship_counts <- rbind(championship_counts, data.frame(Team = winning_team, Championships =
                                                                   1))
  }
  24
}
teams_to_include <- c("Benetton", "Brabham-Repco", "Red Bull", "Ferrari", "McLaren",
                      "Williams", "Mercedes", "Brawn", "BRM", "Cooper-Climax", "Lotus-Climax",
                      "Lotus-Ford", "Vanwall")
...
team_colors <- c("Benetton" = "cyan", "Brabham-Repco" = "purple", "Red Bull" = "navy",
                 "Ferrari" = "red", "McLaren" = "orange", "Williams" = "#a29dfa",
                 "Mercedes" = "#02b0b0", "Brawn" = "#32CD32", "BRM" = "pink", "Cooper-Climax" =
                   "brown",
