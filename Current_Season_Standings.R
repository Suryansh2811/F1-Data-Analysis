#The season dashboard created using Shiny offers a comprehensive overview of the 2024 Formula 1
#season, providing insights into both constructor and driver standings through four distinct graphs.
...
server <- function(input, output) {
  ...
  constructor_data <
    constructor_standings$MRData$StandingsTable$StandingsLists[[1]]$ConstructorStandings
  31
  ...
  fetch_all_race_results <- function() {
    ...
    repeat {
      race_results <- fetch_data(sprintf("/results.json?limit=%and offset=%d", limit, offset))
  ...
  race_df <- bind_rows(lapply(all_races_data, function(race) {
    ...
    bind_rows(lapply(race_results, function(result) {
      ...
      data.frame(Race_Location = race_location, Race_Name = race_name, Driver_Name = driver_name,
                 Position = position)
    }))
  }))
  ...
  avg_finish_position <- race_df %>%
    ...
    summarize(Avg_Finish_Position = mean(Position, na.rm = TRUE))
  ...
  output$overall_constructor_points <- renderPlot({
    ggplot(overall_constructor_points, aes(x = Constructor_Name, y = Average_Points, fill =
                                             Constructor_Name)) +
      ...
      geom_abline(intercept = mean(overall_constructor_points$Average_Points), slope = 0, color = "darkred",
                  linetype = "dashed")
  })
